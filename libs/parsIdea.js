...
for (let page of arrUrl) {
	requestPromise(
		`https://invest-idei.ru/api/ideas/filter?asset=all&iis=0&currency=&growth_factors=&status=any&hot=0&potential_yield=0,101&horizon=0,27&risk=0&broker_id=&stat=0&licenced=0&sort_by=date_start&sort_order=desc&period=all&page=${page}`
	)
		.then(function (html) {
			...
			for (let i of a.data.ideas) {
				...
				if (i.tickers[0].ticker_ticker.includes(".")) {
					continue;
				}
				let a = {
