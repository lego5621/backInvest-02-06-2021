...
module.exports = async function app(z) {
	...
	const statement = await yahooFinance.default.quoteSummary(
		z.ticker,
		{
			modules: [
				"earnings",
	...
	const quote = await yahooFinance.default.quote(z.ticker, {
		fields: [
			"marketCap",
			"regularMarketPrice",
			"priceToBook",
	...
	let recommendationPrice =
		setPrice(price)[setPrice(price).length - 1].adjTargetPrice;
	...
	await translatte(statement.assetProfile.longBusinessSummary, { to: "ru" })
		.then((res) => {
			descriptionChec =
				res.text.substring(0, 220) + res.text.substring(220).split(".")[0];
	...
	const companyData = {
		...
		profitPercentage:
			((recommendationPrice - currentPrice) / recommendationPrice) * 100,
...
function getOtherFactors(result, insights) {
	...
	try {
		obj.redemptionShares =
			Math.abs(
				result.cashflowStatementHistory.cashflowStatements[0].repurchaseOfStock
			) > result.cashflowStatementHistory.cashflowStatements[0].issuanceOfStock;
	} catch (err) {
	...
	try {
		if (
			insights.instrumentInfo.technicalEvents.shortTermOutlook.direction ==
			"Bullish"
		) {
	...
	try {
		if (
			insights.instrumentInfo.technicalEvents.longTermOutlook.direction ==
			"Bullish"
		) {
